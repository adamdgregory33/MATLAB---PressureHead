...
maximise = true;%SET TO FALSE AND CHANGE MAX GEAR RATIO TO TEST OTHER GEAR RATIOS

if( ~maximise)
    maxGearRatio = 0.1;
    
    disp("RPM after gearing: "+maxGearRatio*rpm);
       
    rpm = preRPM*maxGearRatio;
    %RPM = (v/r) * (60/(2*pi) for omega in RPM

    
    torque = preTorque/maxGearRatio;
    ...
    if(isempty(rpmTorqueCrosspoint) == 0)
        ...
        if rpmTorqueCrosspoint(1) > tempMaxRPM
            
            tempMaxRPM = rpmTorqueCrosspoint(1);
            rpmPowerCrosspoint = InterX([tempLinePower(:,1).';tempLinePower(:,2).'],[xValsRpm;yValsRpmPower]);
            %find where the motor curve and this line meet
            ...
            if(isempty(rpmPowerCrosspoint) ==0)
                ...
                plot(tempLinePower(:,1),tempLinePower(:,2),xValsRpm,yValsRpmPower);
                title('power vs rpm');
                axis([0 4100 0 max(yValsRpmPower)]);
                hold on;
...
while maximise
    maxGearRatio = maxGearRatio +increment;
    rpm = preRPM*maxGearRatio;
    %RPM = (v/r) * (60/(2*pi) for omega in RPM
...
disp("Power after gearing: "+tempPower);
disp("RPM after gearing: "+rpm);
disp("Gear Ratio: "+ maxGearRatio);
