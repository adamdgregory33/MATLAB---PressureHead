...
tempMaxPower = 0;
tempMaxPowerDiameter = 0;

counter = 0;
maxPower = 0;
efficiency = 0;
finalDiameters = initialDiameter;
flowPowerVals = 0;

while finished == false
    finalDiameters = horzcat(finalDiameter,finalDiameters);
    counter = counter+1;
    ...
    flowPowerVals = horzcat((gravity*density*operatingPoint(1)*operatingPoint(2)),flowPowerVals);
    
    if tempMaxPower<(operatingPoint(1)*operatingPoint(2))
        tempMaxPower = (operatingPoint(1)*operatingPoint(2));
        tempMaxPowerDiameter = finalDiameter;
    end
    ...
    figure(6)
    title('velocity')
    plot(finalDiameter,tempVelocity,'ro');
    hold on
    ...
    if(isempty(rpmTorqueCrosspoint) == 0)
        ...
        if(isempty(rpmPowerCrosspoint) ==0 && (finalDiameter - decrement) > 0 )
            ...
            figure(9)
            plot(finalDiameter,tempPower,'ro')
            title('Power')
            hold on
            ...
            figure(8)
            plot(finalDiameter,reynoldsNum,'ro')
            title('reynolds num')
            hold on
...
%disp(finalDiameter);
disp("optimal nozzle diameter : "+tempMaxPowerDiameter);
figure(11)
plot(finalDiameters,flowPowerVals,'r-');
title("Figure 2: Power Output vs Nozzle Diameter");
ylabel('Power (W)')
xlabel('Nozzle Outlet Diameter (m)')
axis([0 max(finalDiameters)*1.1 0 max(flowPowerVals)*1.1]);
grid on;
